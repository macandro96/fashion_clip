...
class iMaterialistDataset(data.Dataset):
    def __init__(self, data_type, transforms=None):
        ...
        # image data info
        self.data_dir = '/vast/am10150/fashion_clip/data/raw/imaterialist'
        ...
        # load label info
        f = open(f'/vast/am10150/fashion_clip/data/raw/imaterialist/{data_type}.json', 'r')
        label_data = json.load(f)

        self.annotations = label_data['annotations']

        # id -> label info
        df = pd.read_csv('/vast/am10150/fashion_clip/data/raw/imaterialist/labels.csv')
    ...
    def __getitem__(self, index):
        ...
        label_text = '#'.join([label[0] for label in found_labels])
        attr_text = '#'.join([label[1] for label in found_labels])
        return image, label_text, attr_text, label_tensor
...
if __name__ == '__main__':
    ...
    dataloader = data.DataLoader(dataset, batch_size=16, num_workers=1)

    max_height, max_width = 0, 0
    for i, (images, label_text, attr_text, labels) in tqdm.tqdm(enumerate(dataloader)):
